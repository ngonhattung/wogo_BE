    ...
    @Query(value = """
            ...
            FROM (
               ...
               \s
                UNION ALL
               \s
                -- Lịch sử BOOKING (có user_id, worker_id)
                -- ✅ Lấy cả COMPLETED và CANCELLED
                SELECT
                       'BOOKING' AS type,
            ...
            WHERE
                ...
                -- Nếu là thợ → các booking có worker ứng với user
                OR (:isWorker = true AND worker_id IN (SELECT id FROM worker WHERE user_id = :userId))
            ORDER BY date DESC;
        """, nativeQuery = true)
